...
INIT = Pig.compile("""
A = LOAD 'gs://mon_seau/page_links_en.nt.bz2' using PigStorage(' ') as (url:chararray, p:chararray, link:chararray);
B = GROUP A by url;                                                                                  
...
params = { 'd': '0.5', 'docs_in': 'gs://mon_seau/out/pagerank_data_simple' }
...
for i in range(3):
   out = "gs://mon_seau/out/pagerank_data_" + str(i + 1)
   params["docs_out"] = out
   Pig.fs("rmr " + out)
